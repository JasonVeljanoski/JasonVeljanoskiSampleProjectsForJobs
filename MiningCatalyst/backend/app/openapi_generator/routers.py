...
def save_routers(spec: Specification):
...
def save_tag(tag: str, items: list[PathWrap], spec):
    with open(f"{SERVICES_DIR}/{tag}Router.ts", "w") as f:
        write_imports(f, items)

        f.write(f"export function use{tag}Router() {{\n")

        write_routes(f, items, spec)

        write_return(f, items, spec)

        f.write("}\n")


def write_imports(f, items: list[PathWrap]):
    f.write("import { makeRequest } from '../core/requests'\n\n")
    ...
    for temp in sorted_dependencies:
...
def write_route(f, item: PathWrap, spec):
    ...
    f.write(f"{X(1)}const {path.operation_id} = (\n")
...
def write_args(f, path: Path):
    ...
    f.write(f"{X(2)}aborter_key = null as string | boolean | null \n")
...
def write_function_body(f, method, url, path: Path, spec):
    ...
    date_fields = get_date_fields(path, spec)

    if date_fields:
        date_str = "['" + "', '".join(date_fields) + "']"
        f.write(f"{X(2)}const date_fields = {date_str}\n")
    ...
    if date_fields:
        f.write(f"{X(3)}date_fields,\n")

    f.write(f"{X(2)}}})\n")


def write_return(f, items: list[PathWrap], spec):
    f.write(f"{X(1)}return {{\n")

    for item in items:
        f.write(f"{X(2)}{item.path.operation_id},\n")

    f.write(f"{X(1)}}}\n")


def get_date_fields(path: Path, spec: Specification):
    ...
    for x in possible:
        try:
            ...
            date_string_paths.extend(temp_paths)
