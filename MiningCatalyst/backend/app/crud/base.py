...
import sqlalchemy.ext as sql_ext
...
class CRUD_Base(Generic[T]):
    ...
    def get(self, db: Session, id: int) -> T:
        return db.query(self.schema).filter(self.schema.id == id).first()
    ...
    def get_kw(self, db: Session, order_by=None, **filters) -> list[T]:
        ...
        return query.order_by(order_by or self.order_key).all()
    ...
    def create_kw(self, db: Session, **kwargs) -> T:
        return self.create(db, self.schema(**kwargs))

    def update_all(self, db: Session, edits: list, commit=True) -> list[T]:
        items = [self.update(db, x, commit=False) for x in edits]
    ...
    def update(self, db: Session, edits, commit=True) -> T:
        ...
        for k, v in edits.items():
            ...
            if field_type == sql_orm.attributes.InstrumentedAttribute:
                ...
                # Its a relationship
                    ...
                    temp = field.property
                    sub_crud = getattr(crud, temp.argument.lower())

                    # Other table has the foreign key
                    if temp.direction == sql_orm.relationships.ONETOMANY:
                        sub_items = [sub_crud.update(db, x, commit=False) for x in v]
                    ...
                    # This table has the foreign key
                        ...
                        sub_item = sub_crud.update(db, v, commit=False)
    ...
    def delete(self, db: Session, id: int, in_memory=False):
        return self.delete_kw(db, id=id, in_memory=in_memory)

    def delete_kw(self, db: Session, in_memory=False, **filters):
        query = db.query(self.schema)
