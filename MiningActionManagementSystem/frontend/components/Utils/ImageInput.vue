<template>
  <div>
    <v-dialog v-model="dialog" v-bind="{ ...$bind.dialog }">
      ...
      <div class="dialog-container">
        <v-image-input
          ...
          :image-width="512"
          :image-height="288"
          :success-icon-style="{ color: 'var(--v-primary-base) !important' }"
          :upload-icon-style="{ color: 'var(--v-primary-base) !important' }"
          success-icon="mdi-upload"
          upload-icon="mdi-upload"
          image-format="jpeg"
          clear-icon="mdi-close"
          flip-horizontally-icon="mdi-flip-horizontal"
          flip-vertically-icon="mdi-flip-vertical"
    ...
    <div class="img-wrapper mt-4">
      <div v-for="(img, i) in images" :key="i" class="img-item" :style="imageStyle">
        <v-img :src="img" />
        <e-icon-btn x-small class="remove-img" @click="removeImage(i)"> mdi-close </e-icon-btn>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    total_bytes() {
      let sizeBytes = 0
      for (const meta of this.images_meta) sizeBytes += meta.size
      return sizeBytes
    },
    format_bytes() {
