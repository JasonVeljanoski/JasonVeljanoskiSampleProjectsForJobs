<template>
  <e-dialog v-if="feedback" :dialog="dialog" :title="card_title" persistent scrollable width="1200">
    <template #card:body>
      <v-form ref="form" class="feedback_root">
        <div>
          ...
          <v-textarea
            ...
            :rules="[$form.required(feedback.title), $form.length(feedback.title, 60)]"
            hide-details="auto"
            rows="3"
            counter="60"
            no-resize
          ...
          <v-textarea
            ...
            v-bind="$bind.freetext"
            :rules="[$form.required(feedback.page), $form.length(feedback.page, 50)]"
            hide-details="auto"
            rows="1"
            counter="50"
            no-resize
          ...
          <v-textarea
            ...
            v-bind="$bind.freetext"
            :rules="[$form.required(feedback.summary), $form.length(feedback.summary, 2000)]"
            hide-details="auto"
            rows="4"
            counter="2000"
            no-resize
          ...
          <template v-if="is_bug">
            ...
            <v-textarea
              ...
              v-bind="$bind.freetext"
              :rules="[$form.conditional_required(feedback.replicate, is_bug), $form.length(feedback.replicate, 2000)]"
              hide-details="auto"
              rows="6"
              counter="2000"
              no-resize
        ...
        <div v-if="$perms.is_admin">
          ...
          <span class="d-flex">
            <h4>Admin Notes</h4>
            <small class="ml-2"> (Admins only, This task can be completed later) </small>
          </span>
          <v-textarea
      ...
      <comment-table
        ...
        class="my-4 mx-2"
        @create_comment="addComment($event)"
      />
    </template>
    ...
    <template #card:footer>
      ...
      <v-spacer />
      <save-btn outlined tooltip="Members in the feedback user list will receive an email" @click="submit(true)">
        <v-icon left>mdi-content-save-move</v-icon>
        Save & Email
      </save-btn>
      <v-btn v-bind="$bind.btn" class="pr-4" @click="submit()">
...
export default {
  ...
  computed: {
    current_attachment_bytes() {
      let total = 0
      for (const meta of this.feedback.general_attachments) total += meta.size
    ...
    card_title() {
      if (this.feedback) return this.feedback.id ? 'Edit Feedback' : 'Create Feedback'
      return ''
    },
  },
  methods: {
    ...
    handleNewAttachment(attachment) {
      ...
      const file_clone = {
        ...
        file: new File([attachment.file], attachment.file.name, { type: attachment.file.type }),
      }
