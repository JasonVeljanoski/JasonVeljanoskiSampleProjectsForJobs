<template>
  <v-card outlined elevation="0">
    ...
    >
      ...
      <template #item.title="{ item }">
        <v-tooltip bottom max-width="300">
          <template #activator="{ on, attrs }">
            <span v-bind="attrs" class="attachment-title" v-on="on">
              {{ item.title }}
            </span>
          </template>
    ...
    <!-- DIALOG -->
    <attachment-uploader-dialog ref="uploader" />
  </v-card>
</template>

<script>
import AttachmentUploaderDialog from '@/components/Attachment/AttachmentUploaderDialog.vue'

export default {
  components: {
    AttachmentUploaderDialog,
  },
  props: {
