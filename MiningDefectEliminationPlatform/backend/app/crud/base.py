...
# ! This is still getting designed/implemented
...
class CRUD_Base(Generic[T]):
    ...
    def get_create(self, db: Session, **kwargs):
    ...
    def duplicate(self, db: Session, id) -> T:
        obj = self.get(db, id)
...
def add_date_filter(query, field, column, filters):
    if field in filters:
        ...
        if max_date:
            query = query.filter(column < max_date + dt.timedelta(days=1))
