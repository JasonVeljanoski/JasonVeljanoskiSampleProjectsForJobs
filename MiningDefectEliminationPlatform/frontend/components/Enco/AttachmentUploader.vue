<template>
  <v-dialog v-model="dialog" width="600" persistent>
    ...
    >
      ...
      <!-- FORM -->
      <v-card-text v-if="!hide_form" class="pt-4">
        <v-form ref="attachment_form" class="form">
          ...
          />
          <span class="d-flex">
            <h4>Network Drive Link</h4>
            <small class="ml-2"> (optional) </small>
          </span>
          <v-textarea
            v-model="attachment.network_drive_link"
            v-bind="$bind.select"
            :rules="[$form.length(attachment.network_drive_link, 150)]"
            hide-details="auto"
            counter="150"
            rows="2"
            no-resize
...
export default {
  ...
  methods: {
    open() {
      ...
      this.attachment.network_drive_link = null;
    ...
    save() {
      ...
      // if hide_form is true, skip form validation
      if (this.hide_form) {
    ...
    addFile(files) {
      ...
      // upload by clicking attachment icon
        ...
        const file_ext = files.name.split(".").pop();

        if (this.accept && !(this.accept.includes(files.type) || this.accept.includes(`.${file_ext}`))) {
          this.$snackbar.add("File type not allowed", "warning");
          return;
        }
