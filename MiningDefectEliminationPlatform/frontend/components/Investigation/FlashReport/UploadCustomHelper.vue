<template>
  <v-menu v-model="menu" offset-y min-width="200" :close-on-content-click="false" left>
    ...
    <v-card dense outlined min-width="400">
      <v-card-title>
        <v-checkbox
          v-model="investigation.flash_report.use_custom_report"
          :disabled="loading || non_deleted_attachment_metadata.length === 0"
          label="Use as Flash Report"
...
export default {
  ...
  methods: {
    ...
    handleNewAttachment(attachment) {
      ...
      this.$axios
        .$post("/flash_report/upload", form_data, {
          params: {
    ...
    removeAll() {
      for (let i = 0; i < this.attachment_metadata.length; i++) {
        ...
        this.investigation.flash_report.use_custom_report = false;
      }
    },
    ...
    updateSelected(flag) {
      ...
      this.$axios
        .$$patch("/flash_report/selected", null, {
          params: {
            flash_report_id: this.investigation.flash_report.id,
            filename,
            flag,
        ...
        .then(() => {
          this.investigation.flash_report.use_custom_report = flag;
          // remove extension from filename
          filename = filename.split(".")[0];
          filename = `${filename}.pdf`;
          this.investigation.flash_report.custom_report_fname = flag ? filename : null;
