...
export default {
  ...
  computed: {
    ...
    max_count() {
      let get_length = (item, count) => {
        if (Object.keys(item).length > 0) {
          return (
            1 +
            Math.max(...Object.values(item).map((x) => get_length(x, count)))
          );
        }
      ...
      let temp_root = this.selected_options.reduce(
        (a, b) => a[b],
        this.floc_tree
      );
    ...
    available_floc_indexes() {
      ...
      let available = [...Array(max_count)].map(() => []);
    ...
    is_not_leaf() {
      ...
      for (let key of this.selected_options) tree = tree[key];
  ...
  methods: {
    ...
    submitValue() {
      let new_value = this.selected_options.join("-");
